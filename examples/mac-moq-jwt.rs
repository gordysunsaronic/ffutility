...
//! Read raw video data from stdin, encode to H.264, and stream via MoQ WITH JWT AUTHENTICATION
//! 
//! Usage:
//! ```bash
//! # Webcam streaming (raw YUV420P)
//! ffmpeg -f avfoundation -r 30 -s 1920x1080 -i "0" -f rawvideo -pix_fmt yuv420p - | cargo run --example mac-moq-jwt
//! 
//! # File streaming (raw YUV420P)
//! ffmpeg -stream_loop -1 -i test.mp4 -f rawvideo -pix_fmt yuv420p - | cargo run --example mac-moq-jwt
//! ```
//! 
use anyhow::Result;
